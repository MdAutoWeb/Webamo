...
const diensten: { tag: string; title: string; body: string; href: string }[] = [
  ...
  },
  {
    ...
    body: "Leadmachine met Airtable, WhatsApp-automatisatie en CRM-koppeling, de basis die je leads opvangt en bijhoudt, op maat en ook zonder nieuwe website.",
  ...
  {
    tag: "AI-Agents",
    title: "Onze AI-Agents",
    body: "Offerteflows met goedkeuring, inbox-sortering, support via chatflow uit je kennisbank en maandrapport in gewone taal, op maat.",
    href: "/diensten#agents",
  },
];
  ...
  return (
    <section className="bg-[#F5F5F5]">
      <div className="max-w-[1240px] mx-auto px-5 py-16 md:py-24">
        ...
        <MobileSwipeRow desktopCols="md:grid-cols-2 lg:grid-cols-5" itemWidth="w-[min(85vw,280px)]">
          {diensten.map(({ tag, title, body, href }) => (
            ...
            >
              <Link
                href={href}
                className="group bg-white rounded-[16px] p-5 lg:p-6 border border-[#E5E7EB] hover:border-[#0A0A0A] transition-colors flex flex-col h-full min-w-0"
              >
                <span className="inline-block text-[10px] font-semibold tracking-[0.1em] uppercase text-[#2563EB] bg-[rgba(37,99,235,.08)] px-[9px] py-[4px] rounded-full mb-4 self-start">
                  {tag}
                </span>
                <h3 className="font-semibold text-[16px] lg:text-[15px] tracking-[-0.01em] leading-[1.25] mb-2">
                  {title}
                </h3>
                <p className="text-[13px] text-[#6B7280] leading-[1.55] flex-1 line-clamp-5">
                  {body}
                </p>
                <div className="mt-4 inline-flex items-center gap-[6px] text-[12px] font-medium text-[#2563EB] group-hover:gap-[10px] transition-all">
                  Meer info
